...
class HashKey {
	...
	public function eq(other:HashKey):Bool {
		var i:Int = 0;
...
class HashMap<T> {
    ...
    public function new(powerOf2:Int) {
        ...
        table = [for (_ in 0...mCapacity + 1) new KeyValue<T>()];
    }
	...
	public function add(key:HashKey, value:T):KeyValue<T> {
        ...
        i ^= (hash >>> 16);
        i &= mCapacity;
        var node:KeyValue<T> = table[i];

        var length:Int = 1;
        while (node != null && node.key.get(0) != 0) {
            if (node.hash == hash && key.eq(node.key)) { return node; }
            node = node.next;
            ++length;
        }
